...
2. Mỗi khẳng định phải có trích dẫn ngay sau dưới dạng nhãn trích dẫn [Tên nguồn tài liệu, Năm], ví dụ: [Shopee Returns Policy, 2026] hoặc [luat-phong-chong-ma-tuy.pdf, 2026].
...
def format_context(chunks: list[dict]) -> str:
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        year = metadata.get("year") or "2026"
        doc_type = metadata.get("type", "unknown")
        context_parts.append(
            f"[Nguồn tài liệu: {title} | File: {source} | Năm: {year} | Loại: {doc_type}]\n"
            f"{chunk['content']}\n"
        )
    return "\n---\n".join(context_parts)
